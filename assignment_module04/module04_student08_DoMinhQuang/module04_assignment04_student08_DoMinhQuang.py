...
def distance(pointA: Point, pointB: Point) -> float :#hàm tính khoảng cách giữa 2 điểm
    vector_AB = Point(pointB.x - pointA.x, pointB.y - pointA.y)
...
def evaluate_surface3(pointA: Point, pointB: Point, pointC: Point) -> float :
    vector_AB = Point(pointB.x - pointA.x, pointB.y - pointA.y)
    AB = math.sqrt(vector_AB.x ** 2 + vector_AB.y ** 2) #độ dài cạnh AB
    line = Line.line_general_equation(pointA, pointB) #đường thẳng đi qua A, B
    distance = line.find_distance(pointC) #tìm chiều dài đường cao từ C đến AB
...
B = Point(2,10)
C = Point(2,5)
print(round((evaluate_surface2(A, B, C)), 1))#2.5
print(evaluate_surface1(A, B, C))#2.5
print(evaluate_surface3(A, B, C))#2.5
print(round((evaluete_surface4(A, B, C)), 1))#2.5
