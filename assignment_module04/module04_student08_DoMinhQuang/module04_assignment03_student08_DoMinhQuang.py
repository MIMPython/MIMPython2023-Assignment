...
if __name__ == '__main__':
    ...
    lineA = Line(1, 1, 1)
    lineB = Line(2, 2, 2)
    lineA.find_intersect(lineB)# Infinite roots

    # Nghiem duy nhất -> 2 đường thằng giao nhau
    lineC = Line(2,4,5)
    lineD = Line(2,5,-4)
    lineC.find_intersect(lineD)#(20.5, 9.0)

    # Vô nghiệm -> 2 đường thẳng song song
    lineE = Line(1,1,1)
    lineF = Line(1,1,2)
    lineE.find_intersect(lineF)#()

    #e
    pointA = Point(1,2)
    print(lineA.find_distance(pointA))#2.82842712474619

    #f
    point1 = Point(2, 4)
    point2 = Point(1, 2)
    line = Line.line_general_equation(point1, point2)
    print(f"Phương trình đường thằng là {line.a}x + {line.b}y + {line.c} = 0")
